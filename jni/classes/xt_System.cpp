...
static jstring xtSystem_getDeviceInfo(JNIEnv *env, jclass clazz)
{
    ...
    const char *override_info = getenv("MASSEFFECT_DEVICE_INFO");
    if (override_info) {
    ...
    struct { const char *label; const char *value; } fields[12] = {
        ...
        { "ABI",          "armeabi-v7a"                            },
        { "Build",        "masseffect native loader"                },
    };
 ...
 * a scale of 1 means. Nothing inside libmasseffect.so calls
...
static jstring xtSystem_getInputDeviceNameByDeviceId(JNIEnv *env, jclass clazz, jint device_id)
{
     ...
     *
     * MASSEFFECT_PAD_NAME overrides it, so the other profiles can be tried
     * without a rebuild.
     */
    const char *forced = getenv("MASSEFFECT_PAD_NAME");
    if (forced && *forced)
