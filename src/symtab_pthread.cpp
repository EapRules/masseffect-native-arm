 ...
 * sometimes. MASSEFFECT_LATE_THREAD_MS exists so the next person can find out
...
static bool is_late_thread(void *(*entry)(void *))
{
    ...
    const char *want = getenv("MASSEFFECT_LATE_THREAD_AT");
    if (!want || !*want)
...
{
    const char *env = getenv("MASSEFFECT_LATE_THREAD_MS");
    /*
     ...
     * single thread that matters is what MASSEFFECT_LATE_THREAD_AT is for, and 0
    ...
    trace("  ^ delayed %d ms before starting (engine startup race; "
          "MASSEFFECT_LATE_THREAD_MS=0 disables)", ms);
 ...
 *
 * That is where Mass Effect Infiltrator died, before onCreate and before anything was logged:
 ...
 *
 * (libmasseffect.so ships it at .data+0x1210c0, word = 0x4000), and the engine's
 ...
 * still showing nothing but "Linking libmasseffect.so...".
 *
...
int bionic_pthread_mutex_init(BIONIC_pthread_mutex_t *m, pthread_mutexattr_t **attr)
{
     ...
     *
     * Mass Effect Infiltrator allocates its mutexes with the engine's own allocator, which
     ...
     * word the game reserved. libmasseffect.so imports no pthread_mutexattr_*
