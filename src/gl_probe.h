 ...
 * every GL import resolved to nil and the loader died with the whole import
 * table missing.
 *
 ...
 * the subcommand: the launcher runs `masseffect --gl-probe <library> [symbol
 * ...]` before committing to a candidate, and falls through to the next tier
 * when it fails.
 */
#ifndef MASSEFFECT_GL_PROBE_H
#define MASSEFFECT_GL_PROBE_H
