...
int main(int argc, char **argv)
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER) != 0) {
         ...
         * all. So the context request is the mirror of Dead Space's - ask for
         * ES 2.0 first, which is what the Mali blob on the console and Mesa
         * under the harness both speak natively. Dead Space's ES1-vs-desktop
    ...
    if (gl) {
        load_gles1_funcs();
        /* Dead Space never called this - fixed function, zero shader imports.
