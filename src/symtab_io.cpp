 ...
 *   .../Android/data/com.ea.masseffect/files/ -> that prefix deleted
 *   masseffect/published                      -> masseffect/assets/published
 *
 ...
 * path that still names "masseffect/assets/" anywhere is re-rooted onto the
...
static bool try_writable(const char *path)
{
    ...
    char probe[PATH_MAX];
    snprintf(probe, sizeof(probe), "%s/.masseffect-write-test", path);
...
const char *io_writable_dir(void)
{
    ...
    if (try_writable(getenv("MASSEFFECT_SAVEDIR"))) {
        trace("writable storage: %s (MASSEFFECT_SAVEDIR)", g_writable_dir);
        return g_writable_dir;
    }
    ...
    if (xdg && *xdg) {
        snprintf(candidate, sizeof(candidate), "%s/masseffect", xdg);
        if (try_writable(candidate)) {
    ...
    if (home && *home) {
        ...
        mkdir(candidate, 0777);
        snprintf(candidate, sizeof(candidate), "%s/.local/share/masseffect", home);
        if (try_writable(candidate)) {
    ...
    if (try_writable("/tmp/masseffect")) {
...
static const char kAndroid[]  = "Android/data/com.ea.masseffect/files/";
...
const char *fix_path(const char *orig, char *buf, size_t bufsz)
{
     ...
     *
     * The reference spells it "masseffect/published" -> "masseffect/assets/
     * published" because its data root is a compile-time constant ending in
     * "masseffect". Ours is argv[1], handed to the engine by
    ...
    char *published = strstr(work, "masseffect/published");
    if (published) {
        char tail[PATH_MAX];
        snprintf(tail, sizeof(tail), "%s", published + strlen("masseffect/"));
        snprintf(published, sizeof(work) - (size_t)(published - work),
                 "masseffect/assets/%s", tail);
        changed = true;
    }

    /* The divergence described above: re-root anything still pointing at an
     * asset tree we are not mounted at. */
    char *assets = strstr(work, "masseffect/assets/");
    if (assets) {
        snprintf(buf, bufsz, "%s/%s", g_game_dir,
                 assets + strlen("masseffect/"));
        return buf;
    }
