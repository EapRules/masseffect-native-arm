 ...
 */
extern "C" int masseffect_open(const char *path, int flags, ...)
{
    mode_t mode = 0;
...
extern "C" int masseffect_ioctl(int fd, int request, ...)
{
    va_list ap;
    va_start(ap, request);
 ...
 * impl_tab.h has it commented out as THUNK_MISSING), which is why Minigore 2
 * never needed this: its build of the engine never referenced the symbol.
 * Mass Effect Infiltrator's does, so leaving it unresolved fails the load outright.
 *
 ...
 */
extern "C" void masseffect_assert2(const char *file, int line,
...
DynLibFunction symtable_libm[] = {
    ...
    /* frexp writes the exponent through an int* that crosses unchanged; only
     * the double return needs the bridge. First importer: Mass Effect. */
    ...
    NO_THUNK("open",  (uintptr_t)&masseffect_open),
    NO_THUNK("ioctl", (uintptr_t)&masseffect_ioctl),
    /* No float crosses the boundary, so no ABI bridge is needed. */
    NO_THUNK("__assert2", (uintptr_t)&masseffect_assert2),
