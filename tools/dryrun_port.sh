...
# run: it builds a throwaway PortMaster installation around ports/Mass Effect Infiltrator.sh
...
#   * "/$directory/ports/masseffect" resolves to where the port was installed
#   * cd, the log.txt redirect and mkdir of MASSEFFECT_DATA_DIR all succeed
...
#       masseffect-build tools/dryrun_port.sh /game
set -uo pipefail

DONOR="${1:?usage: dryrun_port.sh <path to Mass Effect Infiltrator APK, ZIP or extracted tree>}"
ROOT=/tmp/dryrun
PM="$ROOT/home/.local/share/PortMaster"
GAMEDIR="/roms/ports/masseffect"
...
cp "build/masseffect"                  "$GAMEDIR/masseffect"
cp "ports/masseffect/masseffect.gptk"   "$GAMEDIR/masseffect.gptk"
cp "ports/masseffect/masseffect.eapx.json" "$GAMEDIR/masseffect.eapx.json"
cp "ports/masseffect/cover.png"        "$GAMEDIR/cover.png"
cp "tools/eapx.py"                    "$GAMEDIR/eapx.py"
cp "ports/Mass Effect Infiltrator.sh"              "/roms/ports/Mass Effect Infiltrator.sh"
...
MISSING=""
covq=$(needed_of "$GAMEDIR/masseffect"); covseen=" "
while [ -n "${covq// /}" ]; do
    covnext=""
    for s in $covq; do
...
chmod -x "$GAMEDIR/masseffect"   # prove the launcher's chmod +x is doing work
...
export MASSEFFECT_FRAME_LIMIT="${MASSEFFECT_FRAME_LIMIT:-40}"

echo "[dryrun] executing the launcher"
bash "/roms/ports/Mass Effect Infiltrator.sh"
RC=$?
...
check "eapx marker written"                       test -f "$GAMEDIR/.eapx-masseffect-data.json"
check "eapx import log written"                   test -s "$GAMEDIR/eapx.log"
check "donor native library imported"             test -f "$GAMEDIR/lib/armeabi/libMassEffect.so"
check "donor content tree imported"               test -d "$GAMEDIR/assets/published"
check "ArkOS artwork normalized from canonical cover" cmp -s "$GAMEDIR/cover.png" "/roms/ports/images/Mass Effect Infiltrator.png"
check "binary made executable"                   test -x "$GAMEDIR/masseffect"
