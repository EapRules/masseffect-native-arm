#!/usr/bin/env python3
"""MCP server for the Mass Effect Infiltrator qemu/Mesa development emulator.

It intentionally uses only Python's standard library so an MCP client can run it
without npm, pip, virtualenvs, or network access. Transport is MCP JSON-RPC over
...
RUNTIME_DIR = Path(
    os.environ.get("MASSEFFECT_EMULATOR_RUNTIME", PORT_DIR / "emulator/runtime")
).resolve()
...
def start_emulator(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["MASSEFFECT_CONTROL_DIR"] = str(RUNTIME_DIR)
    if _time_scale > 1.0:
        env["MASSEFFECT_TIME_SCALE"] = f"{_time_scale:g}"
    if (PORT_DIR / "build/masseffect").exists() and not arguments.get("rebuild"):
        env["MASSEFFECT_EMULATOR_SKIP_BUILD"] = "1"
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "start_emulator",
        "description": "Start the persistent qemu-arm/Mesa Mass Effect Infiltrator emulator and wait until its control channel is ready.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "rebuild": {
...
def _handle(message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        ...
        result = {
            ...
            "serverInfo": {"name": "masseffect-emulator", "version": "0.1.0"},
        }
    elif method == "ping":
        result = {}
