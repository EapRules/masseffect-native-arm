 ...
 * name. MASSEFFECT_ASSET_OVERRIDE points at it; the launcher sets it to
...
{
    const char *dir = getenv("MASSEFFECT_ASSET_OVERRIDE");
    if (!dir || !*dir)
        return false;
...
extern "C" int AAsset_openFileDescriptor(AAsset *asset, off_t *outStart, off_t *outLength)
{
    ...
    if (asset->tmp_fd < 0) {
        char tmpl[] = "/tmp/masseffect_asset_XXXXXX";
        int fd = mkstemp(tmpl);
        if (fd < 0)
            return -1;
