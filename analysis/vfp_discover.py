...
Same hazard as Dead Space: FPSCR LEN/STRIDE writes that qemu-arm honours and
ARMv8 silently ignores (RAZ/WI), so the harness stays green while the console
computes only lane zero. Dead Space had 20 regions with 40 arithmetic
...
an existing one (that second half lives in Dead Space's vfp_coverage.py and
